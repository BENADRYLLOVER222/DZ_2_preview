...
public class UserDaoImpl implements UserDao {
    ...
    public User auth(LoginData loginData) {
        ...
        if (user != null && user.getPassword().equals(loginData.getPassword())) {
            System.out.println("User is authorized" + user.getRole());
        } else {
    ...
    @Transactional
    public List<User> getAllPrivUsers() { //потом проверить этот пиздец
        try {
    ...
    @Transactional
    public List<User> getAllBlacklisted() { //потом проверить этот пиздец
        try {
            Session session = sessionFactory.getCurrentSession();
            String hql = "SELECT u FROM User u WHERE u.role = :role";
